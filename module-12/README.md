...
4. **Test the scorer against an existing log** before saving. The scorer editor has a **Test** panel that lets you pick a recent `run_skill` log and run the prompt against its output. Verify the model returns one of `ACTIONABLE` / `PARTIAL` / `NOT_ACTIONABLE` and the mapped score looks right. If it returns prose or a different label, fix the prompt now — once the automation rule is on, a broken prompt produces null scores for every incoming log.

5. Save the scorer.

6. **Create the automation rule** that applies the scorer to incoming logs:
